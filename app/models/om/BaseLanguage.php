...
abstract class BaseLanguage extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the dokeos_folder field.
	 * @var        string
	 */
	...
	/**
	 * Get the [dokeos_folder] column value.
	 * 
	 * @return     string
	 */
	...
	public function setOriginalName($v)
	{
		...
		if ($this->original_name !== $v) {
			...
			$this->modifiedColumns[] = LanguagePeer::ORIGINAL_NAME;
		}
	...
	public function setEnglishName($v)
	{
		...
		if ($this->english_name !== $v) {
			$this->english_name = $v;
			$this->modifiedColumns[] = LanguagePeer::ENGLISH_NAME;
		}
	...
	public function setIsocode($v)
	{
		...
		if ($this->isocode !== $v) {
			$this->isocode = $v;
			$this->modifiedColumns[] = LanguagePeer::ISOCODE;
		}
	...
	/**
	 * Set the value of [dokeos_folder] column.
	 * 
	 * @param      string $v new value
	 * @return     Language The current object (for fluent API support)
	 */
	public function setDokeosFolder($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->dokeos_folder !== $v) {
			$this->dokeos_folder = $v;
			$this->modifiedColumns[] = LanguagePeer::DOKEOS_FOLDER;
		}
	...
	public function setAvailable($v)
	{
		...
		if ($this->available !== $v || $this->isNew()) {
			$this->available = $v;
			$this->modifiedColumns[] = LanguagePeer::AVAILABLE;
		}
	...
	public function setParentId($v)
	{
		...
		if ($this->parent_id !== $v) {
			$this->parent_id = $v;
			$this->modifiedColumns[] = LanguagePeer::PARENT_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->isocode = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->dokeos_folder = ($row[$startcol + 4] !== null) ? (string) $row[$startcol + 4] : null;
			$this->available = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(LanguagePeer::ID) ) {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[4], $arr)) $this->setDokeosFolder($arr[$keys[4]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(LanguagePeer::ORIGINAL_NAME)) $criteria->add(LanguagePeer::ORIGINAL_NAME, $this->original_name);
		if ($this->isColumnModified(LanguagePeer::ENGLISH_NAME)) $criteria->add(LanguagePeer::ENGLISH_NAME, $this->english_name);
		if ($this->isColumnModified(LanguagePeer::ISOCODE)) $criteria->add(LanguagePeer::ISOCODE, $this->isocode);
		if ($this->isColumnModified(LanguagePeer::DOKEOS_FOLDER)) $criteria->add(LanguagePeer::DOKEOS_FOLDER, $this->dokeos_folder);
		if ($this->isColumnModified(LanguagePeer::AVAILABLE)) $criteria->add(LanguagePeer::AVAILABLE, $this->available);
		if ($this->isColumnModified(LanguagePeer::PARENT_ID)) $criteria->add(LanguagePeer::PARENT_ID, $this->parent_id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setIsocode($this->isocode);
		$copyObj->setDokeosFolder($this->dokeos_folder);
