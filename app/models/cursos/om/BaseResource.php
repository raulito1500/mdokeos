...
abstract class BaseResource extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the source_type field.
	 * @var        string
	 */
	...
	/**
	 * Get the [source_type] column value.
	 * 
	 * @return     string
	 */
	...
	/**
	 * Set the value of [source_type] column.
	 * 
	...
	public function setSourceType($v)
	{
		...
		if ($this->source_type !== $v) {
			$this->source_type = $v;
			$this->modifiedColumns[] = ResourcePeer::SOURCE_TYPE;
		}
	...
	public function setSourceId($v)
	{
		...
		if ($this->source_id !== $v) {
			$this->source_id = $v;
			$this->modifiedColumns[] = ResourcePeer::SOURCE_ID;
		}
	...
	public function setResourceType($v)
	{
		...
		if ($this->resource_type !== $v) {
			...
			$this->modifiedColumns[] = ResourcePeer::RESOURCE_TYPE;
		}
	...
	public function setResourceId($v)
	{
		...
		if ($this->resource_id !== $v) {
			$this->resource_id = $v;
			$this->modifiedColumns[] = ResourcePeer::RESOURCE_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->source_type = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->source_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->resource_type = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->resource_id = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->resetModified();
			...
			return $startcol + 5; // 5 = ResourcePeer::NUM_COLUMNS - ResourcePeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(ResourcePeer::ID) ) {
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true)
	{
		...
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getSourceType(),
			$keys[2] => $this->getSourceId(),
			$keys[3] => $this->getResourceType(),
			$keys[4] => $this->getResourceId(),
		);
		return $result;
	}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setSourceType($arr[$keys[1]]);
		if (array_key_exists($keys[2], $arr)) $this->setSourceId($arr[$keys[2]]);
		if (array_key_exists($keys[3], $arr)) $this->setResourceType($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setResourceId($arr[$keys[4]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ResourcePeer::SOURCE_TYPE)) $criteria->add(ResourcePeer::SOURCE_TYPE, $this->source_type);
		if ($this->isColumnModified(ResourcePeer::SOURCE_ID)) $criteria->add(ResourcePeer::SOURCE_ID, $this->source_id);
		if ($this->isColumnModified(ResourcePeer::RESOURCE_TYPE)) $criteria->add(ResourcePeer::RESOURCE_TYPE, $this->resource_type);
		if ($this->isColumnModified(ResourcePeer::RESOURCE_ID)) $criteria->add(ResourcePeer::RESOURCE_ID, $this->resource_id);
	...
	{
		$copyObj->setSourceType($this->source_type);
