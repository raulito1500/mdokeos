...
abstract class BaseCalendarEvent extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the parent_event_id field.
	 * @var        int
	 */
	...
	/**
	 * Initializes internal state of BaseCalendarEvent object.
	 * @see        applyDefaults()
	 */
	...
	/**
	 * Get the [parent_event_id] column value.
	 * 
	 * @return     int
	 */
	...
	public function setContent($v)
	{
		...
		if ($this->content !== $v) {
			$this->content = $v;
			$this->modifiedColumns[] = CalendarEventPeer::CONTENT;
		}
	...
	public function setStartDate($v)
	{
		...
		if ( $this->start_date !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = CalendarEventPeer::START_DATE;
			}
	...
	public function setEndDate($v)
	{
		...
		if ( $this->end_date !== null || $dt !== null ) {
					...
					)
			{
				...
				$this->modifiedColumns[] = CalendarEventPeer::END_DATE;
			}
		} // if either are not null
	...
	/**
	 * Set the value of [parent_event_id] column.
	 * 
	 * @param      int $v new value
	 * @return     CalendarEvent The current object (for fluent API support)
	 */
	public function setParentEventId($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->parent_event_id !== $v) {
			$this->parent_event_id = $v;
			$this->modifiedColumns[] = CalendarEventPeer::PARENT_EVENT_ID;
		}
	...
	public function setSessionId($v)
	{
		...
		if ($this->session_id !== $v || $this->isNew()) {
			$this->session_id = $v;
			$this->modifiedColumns[] = CalendarEventPeer::SESSION_ID;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->parent_event_id = ($row[$startcol + 5] !== null) ? (int) $row[$startcol + 5] : null;
			...
			return $startcol + 7; // 7 = CalendarEventPeer::NUM_COLUMNS - CalendarEventPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(CalendarEventPeer::ID) ) {
					...
					$affectedRows = CalendarEventPeer::doUpdate($this, $con);
				}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[5], $arr)) $this->setParentEventId($arr[$keys[5]]);
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(CalendarEventPeer::CONTENT)) $criteria->add(CalendarEventPeer::CONTENT, $this->content);
		if ($this->isColumnModified(CalendarEventPeer::START_DATE)) $criteria->add(CalendarEventPeer::START_DATE, $this->start_date);
		if ($this->isColumnModified(CalendarEventPeer::END_DATE)) $criteria->add(CalendarEventPeer::END_DATE, $this->end_date);
		if ($this->isColumnModified(CalendarEventPeer::PARENT_EVENT_ID)) $criteria->add(CalendarEventPeer::PARENT_EVENT_ID, $this->parent_event_id);
		if ($this->isColumnModified(CalendarEventPeer::SESSION_ID)) $criteria->add(CalendarEventPeer::SESSION_ID, $this->session_id);
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setParentEventId($this->parent_event_id);
