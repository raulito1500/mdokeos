...
/**
 * Base class that represents a row from the 'survey_answer' table.
 *
 * 
 *
...
abstract class BaseSurveyAnswer extends BaseObject  implements Persistent
{
	...
	public function setAnswerId($v)
	{
		...
		if ($this->answer_id !== $v) {
			$this->answer_id = $v;
			$this->modifiedColumns[] = SurveyAnswerPeer::ANSWER_ID;
		}
	...
	public function setSurveyId($v)
	{
		...
		if ($this->survey_id !== $v) {
			$this->survey_id = $v;
			$this->modifiedColumns[] = SurveyAnswerPeer::SURVEY_ID;
		}
	...
	public function setQuestionId($v)
	{
		...
		if ($this->question_id !== $v) {
			$this->question_id = $v;
			$this->modifiedColumns[] = SurveyAnswerPeer::QUESTION_ID;
		}
	...
	public function setOptionId($v)
	{
		...
		if ($this->option_id !== $v) {
			$this->option_id = $v;
			$this->modifiedColumns[] = SurveyAnswerPeer::OPTION_ID;
		}
	...
	public function setValue($v)
	{
		...
		if ($this->value !== $v) {
			$this->value = $v;
			$this->modifiedColumns[] = SurveyAnswerPeer::VALUE;
		}
	...
	public function setUser($v)
	{
		...
		if ($this->user !== $v) {
			$this->user = $v;
			$this->modifiedColumns[] = SurveyAnswerPeer::USER;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->answer_id = ($row[$startcol + 0] !== null) ? (int) $row[$startcol + 0] : null;
			$this->survey_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			$this->question_id = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->option_id = ($row[$startcol + 3] !== null) ? (string) $row[$startcol + 3] : null;
			$this->value = ($row[$startcol + 4] !== null) ? (int) $row[$startcol + 4] : null;
			$this->user = ($row[$startcol + 5] !== null) ? (string) $row[$startcol + 5] : null;
			$this->resetModified();
			...
			return $startcol + 6; // 6 = SurveyAnswerPeer::NUM_COLUMNS - SurveyAnswerPeer::NUM_LAZY_LOAD_COLUMNS).
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(SurveyAnswerPeer::ANSWER_ID) ) {
						throw new PropelException('Cannot insert a value for auto-increment primary key ('.SurveyAnswerPeer::ANSWER_ID.')');
					}

					$pk = BasePeer::doInsert($criteria, $con);
					$affectedRows = 1;
					$this->setAnswerId($pk);  //[IMV] update autoincrement primary key
					$this->setNew(false);
				} else {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[3], $arr)) $this->setOptionId($arr[$keys[3]]);
		if (array_key_exists($keys[4], $arr)) $this->setValue($arr[$keys[4]]);
		if (array_key_exists($keys[5], $arr)) $this->setUser($arr[$keys[5]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SurveyAnswerPeer::ANSWER_ID)) $criteria->add(SurveyAnswerPeer::ANSWER_ID, $this->answer_id);
		if ($this->isColumnModified(SurveyAnswerPeer::SURVEY_ID)) $criteria->add(SurveyAnswerPeer::SURVEY_ID, $this->survey_id);
		if ($this->isColumnModified(SurveyAnswerPeer::QUESTION_ID)) $criteria->add(SurveyAnswerPeer::QUESTION_ID, $this->question_id);
		if ($this->isColumnModified(SurveyAnswerPeer::OPTION_ID)) $criteria->add(SurveyAnswerPeer::OPTION_ID, $this->option_id);
		if ($this->isColumnModified(SurveyAnswerPeer::VALUE)) $criteria->add(SurveyAnswerPeer::VALUE, $this->value);
		if ($this->isColumnModified(SurveyAnswerPeer::USER)) $criteria->add(SurveyAnswerPeer::USER, $this->user);
	...
	{
		$criteria = new Criteria(SurveyAnswerPeer::DATABASE_NAME);
		$criteria->add(SurveyAnswerPeer::ANSWER_ID, $this->answer_id);
	...
	/**
	 * Generic method to set the primary key (answer_id column).
	 *
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setOptionId($this->option_id);
